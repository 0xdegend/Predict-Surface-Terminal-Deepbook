...
function BinaryBody({
  ...
  // Slide to adjust the strike right on the surface. The popover reads the strike
  // straight from the store selection, so we publish the new strike there — the
  // surface highlight and odds track it in lockstep, same as a node click.
  function applyStrike(next: bigint) {
    if (!oracle || next <= 0n) return;
    select({
      oracleId: oracle.oracle_id,
      expiry: oracle.expiry,
    ...
    const tx =
      feeBps > 0
        ...
        : buildMintTx({
            ...
            isUp,
            quantity: fresh.quantity,
            depositAmount: fundingSplit(fresh.mintCost, acct.tradingBalanceBase)
              .depositAmount,
          });
    const digest = await acct.runTx("mint", tx, [
      ...acct.managerKeys,
  ...
  if (!oracle || !onActive) {
    return (
      <div className="px-3.5 py-3 text-[11px] text-text-3">Loading market…</div>
    );
  }
  ...
  return (
    <div className="flex flex-col">
      <PopoverHeader
        oracle={oracle.underlying_asset}
        expiry={oracle.expiry}
        now={now}
        expired={expired}
      />
      ...
      <div className="flex items-end justify-between px-3.5 pt-3">
        <div className="flex flex-col gap-1">
          <span className="eyebrow">
            {isUp ? "Settles above" : "Settles below"}
          </span>
        ...
        {fairUp != null && (
          <div className="flex flex-col items-end gap-0.5 font-mono text-[11px] tabular-nums">
            <span className={isUp ? "text-up" : "text-text-3"}>
              UP {pct(fairUp, 1)}
            </span>
            <span className={!isUp ? "text-down" : "text-text-3"}>
              DN {pct(1 - fairUp, 1)}
            </span>
          </div>
        )}
      ...
      {view === "glance" ? (
        <div className="flex flex-col gap-2 px-3 pb-3 pt-3">
          {/* Slide to adjust the strike without re-clicking the surface — bounded
              to the quotable band, centered on today's price. */}
...
function RangeBody({
  ...
  async function handleMint() {
    ...
    if (digest) {
      pulseFill({
        oracleId: oracle.oracle_id,
        strike: (band.lower + band.higher) / 2,
        isUp: true,
      });
      onClose();
    }
...
function QuoteCard({
  ...
  return (
    <div
      className={`glass-card p-3 font-mono text-[11px] tabular-nums ${glow}`}
    >
      {body}
    </div>
  );
