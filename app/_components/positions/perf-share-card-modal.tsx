...
import {
  ...
  PERF_SHARE_VARIANTS,
  type PerfShareData,
  type PerfShareVariant,
} from './perf-share-card-canvas';

/**
 * Share-as-image dialog for a trader's settled track record (the Performance
 * bento). Mirrors the position ShareCardModal: renders the card on a canvas, lets
 * the user pick a style, previews it large, and offers download / clipboard / X.
 *
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const v of PERF_SHARE_VARIANTS) {
        ...
        if (el) drawPerfShareCard(el, data, { variant: v.id, scale: 0.5 });
      }
    })();
    return () => {
      cancelled = true;
