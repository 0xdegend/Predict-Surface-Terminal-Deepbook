      ...
      <div className="@container flex flex-col gap-3 p-3.5 sm:p-4">
        ...
        <div className="flex items-center justify-between">
          <span className="flex items-center gap-2">
            <span
              className={`h-1.5 w-1.5 rounded-full ${up ? "bg-up" : "bg-down"}`}
            />
        ...
        <div className="glass-inset flex flex-wrap items-center justify-between gap-x-4 gap-y-3 p-4">
          ...
          <div className="flex items-center gap-3.5">
            <div className="flex flex-col items-end gap-1">
              ...
              <span
                className={`flex items-baseline gap-1.5 ${positive ? "text-up" : "text-down"}`}
              >
        ...
        <div className="flex flex-col gap-3 px-1">
          <div className="grid grid-cols-2 gap-x-4 gap-y-3 @lg:grid-cols-4">
            <Metric
              icon={LuLayers}
              label="Size"
              value={fmtQuote(m.contracts)}
              sub="contracts"
            />
            <Metric
              icon={LuPercent}
              label="Avg entry"
              value={pct(m.entryPrice, 1)}
              sub="implied"
            />
            <Metric
              icon={LuTarget}
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <span className="flex items-baseline gap-1.5">
        <span className="text-[16px] leading-none tracking-tight text-text-1">
          {value}
        </span>
...
function MiniMetric({
  ...
  const inner = (
    <>
      ...
      <span
        className={`flex items-center gap-1 whitespace-nowrap text-[12px] ${valueColor}`}
      >
        {value}
        {href && (
          <LuExternalLink
            size={11}
            className="text-text-3 transition-colors group-hover/mini:text-text-2"
          />
        )}
      </span>
    </>
...
function ResultChip({ result }: { result: "live" | "won" | "lost" }) {
  ...
  return (
    ...
    >
      <span
        className={`h-1.5 w-1.5 rounded-full ${won ? "bg-accent" : "bg-down"}`}
      />
      {won ? "Won" : "Lost"}
    </span>
  );
