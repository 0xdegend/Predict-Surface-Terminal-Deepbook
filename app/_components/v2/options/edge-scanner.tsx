 ...
 * (`upFair`) + empirical (terminal hit-rate) the ladder shows, so the two can't
 * disagree. Honest by construction: it's a recent base rate, not a forecast (see
 * the footnote), and the sample count backs every row.
 */
...
export function OptionsEdgeScanner({
  ...
  const pool = useMemo(() => {
    const scanMarkets: EdgeScanMarket[] = markets.flatMap((m) => {
      ...
      return p ? [{ marketId: m.expiry_market_id, expiryMs: m.expiry, admissionTickSize: m.admission_tick_size, pricer: { forward: p.forward, svi: p.svi } }] : [];
    });
  ...
  }, [markets, pricers, closes, scanNow]);
  ...
  return (
    <section>
      <div className="mb-3 mt-1 flex items-center gap-2.5">
        ...
        <span className="text-[10.5px] uppercase tracking-wide text-text-3">where the surface is cheap vs recent history</span>
      ...
      <div className="glass overflow-x-auto rounded-lg p-4">
        {!closes ? (
          ...
          <div className="py-8 text-center text-[13px] text-text-3">
            The surface is priced in line with recent history right now — no standout edges. Check back as the tape moves.
          </div>
        ) : (
          <table className="w-full min-w-[720px] border-collapse">
            <thead>
              <tr className="text-[10.5px] uppercase tracking-wide text-text-3">
                ...
                <SortableTh label={<Term plain="Edge" pro="Edge (pts)" />} k="edge" sort={sort} onClick={toggle} />
        ...
        <div className="mt-3 flex flex-wrap justify-between gap-2 text-[11.5px] text-text-3">
          <span>
            <Term
              plain='"Edge" = how many points cheaper the surface prices this than moves like it have paid off lately.'
              pro="Edge = empirical terminal hit-rate − surface implied, in probability points."
            />
          </span>
...
function sortCandidates(pool: EdgeCandidate[], sort: { key: SortKey; dir: 'asc' | 'desc' }): EdgeCandidate[] {
  const val = (c: EdgeCandidate): number =>
    sort.key === 'edge' ? c.edgePts
    : sort.key === 'ev' ? c.evPct
    : sort.key === 'payout' ? c.payout
...
function Row({
  ...
  return (
    ...
    >
      ...
      <td className="px-3.5 py-2.5 text-right tabular-nums text-text-2" title={`${c.samples.toLocaleString('en-US')} recent windows`}>
        {(c.empirical * 100).toFixed(0)}%
      </td>
      <td className="px-3.5 py-2.5 text-right font-semibold tabular-nums text-up">+{c.edgePts.toFixed(0)} pts</td>
      <td className="px-3.5 py-2.5 text-right tabular-nums text-up">{signed(c.evPct, 0)}%</td>
      <td className="px-3.5 py-2.5 text-right tabular-nums">{c.payout.toFixed(2)}×</td>
