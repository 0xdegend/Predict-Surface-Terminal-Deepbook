...
/**
 * V2AnalyticsMarketTable — the dense, quant-leaning Markets view for the new
 * deployment (legacy MarketTable's twin): every live market a row with money bet,
 * open bets, expected swing, and crowd mood side by side, sorted by the active
 * metric, with heat bars for at-a-glance scale. Tap a row to bet it.
 */
...
export function V2AnalyticsMarketTable({
  cells,
  metric,
  onTrade,
}: {
        ...
        <div className="rows-divided">
          {sorted.map((c) => {
            ...
            return (
              ...
              >
                <span className="min-w-0 truncate font-mono text-[12px] text-text-1">BTC {num(c.forward, 0)}</span>
                ...
                <span className="text-right font-mono text-[12px] tabular-nums text-text-2">
                  {c.oi > 0 ? compact(c.oi) : <span className="text-text-3">—</span>}
                </span>
                <span className="text-right font-mono text-[12px] tabular-nums text-text-2">
                  {c.atmIv > 0 ? pct(c.atmIv, 0) : <span className="text-text-3">—</span>}
                </span>
