/**
 * SharedTradeLanding — the recipient landing card for a shared trade, shared by the
 * long link (/t/[token]) and the short link (/s/[id]). Server component: it only
 * renders the card from a decoded recipe (or an invalid-link fallback) and hands off
 * to the client OpenSharedTrade button. `sharedTradeMetadata` builds the matching
 * page + OG metadata so both routes stay in sync.
 */
import type { Metadata } from 'next';
import Link from 'next/link';
import { recipeLabel, type TradeRecipe } from '@/lib/share/trade-link';
import { CADENCE_LABEL } from '@/lib/markets/v2-discovery';
import { OpenSharedTrade } from '@/app/t/[token]/open-trade';
...
/** Page + OG metadata for a shared trade (or a neutral title when the link is bad). */
export function sharedTradeMetadata(recipe: TradeRecipe | null): Metadata {
  ...
  const title = `${label} · Skew`;
  const description = `${who} on Skew: ${label}. Open it and place it in a tap.`;
  return {
    title,
    description,
...
export function SharedTradeLanding({ recipe }: { recipe: TradeRecipe | null }) {
  if (!recipe) {
    return (
