 ...
 *
 * Clicking a node anchors this card centered over the canvas in a lightweight
 * GLANCE state (Up/Down toggle + strike slider + fair odds); "Preview trade"
 * expands to the TICKET state (stake + leverage presets, cost estimate, Review
 * → MintConfirmModal → mint). Range mode builds a band from two surface clicks.
 *
 * It is a thin consumer of the SAME synchronous pricing the rail ticket uses —
 * entry probability off the live SVI/forward, `quantityForStake` sizing, the
 * on-chain max_cost / max_probability guards — and the same shared store, so
 * the rail and this popover never disagree on the trade. Cost is an estimate
...
export function SurfaceTradePopoverV2({
  ...
  return (
    <div
      ref={ref}
      className="popover-in glass pointer-events-auto absolute left-1/2 top-1/2 z-20 max-h-[calc(100%-1.5rem)] w-76 max-w-[calc(100vw-1.5rem)] -translate-x-1/2 -translate-y-1/2 overflow-y-auto overflow-x-hidden rounded-xl font-mono text-[12px] tabular-nums shadow-[0_18px_48px_-12px_rgba(0,0,0,0.8)]"
    >
      <button
        type="button"
        onClick={onClose}
      ...
      {mode === 'range' ? (
        ...
        <BinaryBody market={market} input={input} now={now} onClose={onClose} />
      )}
    </div>
  );
}
