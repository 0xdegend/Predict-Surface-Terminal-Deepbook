  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-5">
      ...
      <div className="rise mb-6">
        <h1 className="flex items-center gap-2.5 text-[24px] font-semibold tracking-tight text-text-1">
          <LuBookOpen size={22} className="text-[var(--accent)]" />
          Docs
        </h1>
        <p className="mt-2 max-w-2xl text-[14px] leading-relaxed text-text-2">
          New to Skew? You&apos;re in the right place. This guide explains, in
          plain English, how to make your first trade, how to read the live
          price map, and how everything works. Skew runs on the Sui Testnet, so
          you trade with test tokens that have no real value nothing here costs
          real money.
        </p>
      </div>
      ...
      <div className="glass scroll-quiet sticky top-16 z-30 -mx-4 mb-5 flex gap-1 overflow-x-auto rounded-none border-x-0 px-4 py-2 lg:hidden">
        {SECTIONS.map((s) => (
          <button
            key={s.id}
            onClick={() => jump(s.id)}
            className={`flex-none rounded-md px-2.5 py-1.5 text-[12px] font-medium tracking-tight transition-colors ${
              active === s.id
            ...
            {SECTIONS.map((s) => {
              ...
              return (
                <button
                  key={s.id}
                  onClick={() => jump(s.id)}
                  className={`group flex items-center gap-2.5 rounded-md px-2.5 py-2 text-left text-[13px] font-medium tracking-tight transition-colors ${
                    on
                      ? "bg-[var(--accent-soft)] text-text-1"
                ...
                >
                  <Icon
                    size={14}
                    className={`flex-none ${on ? "text-accent" : "text-text-3"}`}
                  />
                  {s.label}
                </button>
              );
...
function GettingStarted() {
  return (
    <Section id="start" icon={LuRocket} title="Getting started" hue={HUE.teal}>
      <Lead>
        Skew is <b className="text-text-1">more than a yes-or-no market</b>. You
        can bet which way a price (like Bitcoin) will move in the next few
        minutes, bet that it lands inside a price range, or even be the house
        and earn fees. Here&apos;s how to make your first trade:
      </Lead>
      <Flow>
        <FlowStep n={1} title="Connect a wallet">
          A <b className="text-text-1">wallet </b> is your login and your
          account rolled into one. Tap the button at the top-right. The easiest
        ...
        <FlowStep n={3} title="Pick any market to bet on">
          Click any point on the live map (explained next), or choose a market
          from the list. Your bet slip fills in with the details.
        </FlowStep>
...
function ReadingSurface() {
  return (
    <Section id="surface" icon={LuBoxes} title="Reading the map" hue={HUE.blue}>
      <Lead>
        The big 3D shape in the middle is a{" "}
        <b className="text-text-1">live map of every bet you can make</b>.
        Instead of a long, boring list, Skew lays them out as a landscape you
        can explore. The three directions each mean something:
      </Lead>
      <Cards>
        <Card icon={LuTarget} hue={HUE.blue} title="Left ↔ right: the price">
          This is the price level your bet is about, for example, &ldquo;will
          Bitcoin be above $65,000?&rdquo;
        </Card>
        <Card
          icon={LuClock}
          hue={HUE.violet}
          title="Front ↔ back: the deadline"
        >
          How soon the bet is decided. Skew&apos;s markets are fast, many finish
          in about 15 minutes.
        </Card>
        <Card icon={LuSparkles} hue={HUE.amber} title="Up & color: how jumpy">
          The height (and color, cool to warm) shows how big a price swing the
          market expects. Taller and warmer means the market thinks the price
          could move a lot.
        </Card>
      </Cards>
      <SubHead icon={LuMousePointerClick} title="Hover and click" />
      <P>
        Move your pointer over any point to see its details the price level, the
        odds for each side, and the deadline. Click a point to load that exact
        bet into your slip. The map is the controls: what you see is what you
        trade.
      </P>
      <SubHead icon={LuHistory} title="Rewind time" />
      <P>
        Drag the time slider to replay how the map looked a little while ago it
        morphs smoothly so you can watch how the odds shifted. Press{" "}
        <Mono>LIVE</Mono> to jump back to right now.
      </P>
      ...
      <P>
        Prefer a normal price chart? Use the switch at the top-left of the map
        to flip between the 3-D <Mono>Surface</Mono> map and a regular{" "}
        <Mono>Chart</Mono>.
      </P>
...
function Instruments() {
  return (
    ...
    >
      <Lead>
        There are two kinds of bets. Both pay you a fixed{" "}
        <Mono>$1 for each unit</Mono> you hold when you win and you decide how
        many units to buy (that&apos;s your stake).
      </Lead>
      <Cards>
        <Card icon={LuArrowUp} hue={HUE.teal} title="Up or Down">
          A simple direction bet on one price level. Pick{" "}
          <b className="text-text-1">UP</b> if you think the price will finish
          above it, or <b className="text-text-1">DOWN</b> if you think
          it&apos;ll finish at or below. Win and each unit pays $1; lose and it
          pays nothing.
        </Card>
...
function Minting() {
  return (
    <Section
      id="minting"
      icon={LuCoins}
      title="Placing & cashing out"
      hue={HUE.amber}
    >
      <Lead>
        Placing a bet is one quick step, even though a few things happen behind
        the scenes. When you tap the button:
      </Lead>
      <Flow>
        <FlowStep n={1} title="Your account is set up">
          The very first time, Skew creates your trading account automatically.
          You don&apos;t have to do anything.
        </FlowStep>
        <FlowStep n={2} title="Your bet is placed">
          Your coins and your bet are handled together in a single confirmation
          you only sign once.
        </FlowStep>
        ...
        <FlowStep n={4} title="Cash out">
          Close a bet whenever you like. Once a market reaches its deadline,
          winning bets can be paid out automatically. Skew can even collect them
          for you.
        </FlowStep>
      </Flow>
      <Callout tone="accent" title="No surprises on price">
        The price you&apos;re shown comes straight from the live market the
        instant before you confirm so the amount you see is exactly what you
        pay. It&apos;s never a rough guess.
      </Callout>
    </Section>
  );
}
...
function Fees() {
  return (
    <Section id="fees" icon={LuReceipt} title="What it costs" hue={HUE.teal}>
      <Lead>
        Your bet slip always splits the total into two clear lines, so you know
...
function VaultRisk() {
  return (
    ...
    >
      <Lead>
        Every bet is paid out from a shared pool of money. Instead of betting,
        you can put money <b className="text-text-1">into </b> that pool,
        basically becoming &ldquo;the house&rdquo; and earn a share of the fees
        as people trade.
      </Lead>
      <Cards>
    ...
    <Section id="ranks" icon={LuTrophy} title="Ranks & rewards" hue={HUE.amber}>
      ...
      <Cards>
        ...
        <Card icon={LuSparkles} hue={HUE.teal} title="Points">
          ...
          bets do (you&apos;re never penalized for losses), and how long you
    ...
    <Section id="faq" icon={LuCircleHelp} title="FAQ & help" hue={HUE.coral}>
      <div className="flex flex-col gap-2.5">
        ...
        <QA q="What are network fees, and do I pay them?">
          Sui (the network Skew runs on) charges a tiny fee for each action,
          paid in SUI. If you sign in with Google, we have it covered for you.
          If you use your own wallet you pay them, and brand-new wallets get a
          little SUI to start.
        </QA>
        <QA q="I saw “market just expired” or a fee error.">
          The market you picked just reached its deadline or refreshed. Click a
          point on the map again to load a current one. If it keeps happening,
          check that your wallet is on Testnet, some are on Mainnet by default.
        </QA>
        <QA q="Can I use Skew on my phone?">
          Yes. Tap a market to open the bet slip. The 3D map is easiest to
          explore on a bigger screen, but placing bets, cashing out, and your
          portfolio all work on a phone.
        </QA>
        <QA q="Is any of this real money?">
          No. Skew runs on Testnet with test tokens that have no value.
  ...
  return (
    <section id={id} className="scroll-mt-32 lg:scroll-mt-24">
      <div className="mb-4 flex items-center gap-2.5">
        ...
        <h2 className="text-[19px] font-semibold tracking-tight text-text-1">
          {title}
        </h2>
      </div>
...
function Lead({ children }: { children: React.ReactNode }) {
  return <p className="text-[15px] leading-relaxed text-text-2">{children}</p>;
}

function P({ children }: { children: React.ReactNode }) {
  return <p className="text-[14px] leading-relaxed text-text-2">{children}</p>;
}

function SubHead({ icon: Icon, title }: { icon: IconType; title: string }) {
  return (
    <h3 className="mt-1 flex items-center gap-2 text-[15px] font-semibold tracking-tight text-text-1">
  ...
  return (
    <div className="glass-card flex flex-col gap-2.5 rounded-2xl p-4">
      <div className="flex items-center gap-2.5">
        ...
        <h4 className="text-[14px] font-semibold tracking-tight text-text-1">
          {title}
        </h4>
      </div>
      <p className="text-[13px] leading-relaxed text-text-2">{children}</p>
    </div>
  );
}
...
function FlowStep({
  ...
  return (
    <div className="glass-inset flex items-start gap-3 rounded-xl p-3.5">
      <span
        className="flex h-6 w-6 flex-none items-center justify-center rounded-full font-mono text-[12px] font-semibold text-[var(--accent)]"
        style={{
...
function QA({ q, children }: { q: string; children: React.ReactNode }) {
  return (
    <div className="glass-card rounded-2xl p-4">
      <h4 className="flex items-start gap-2 text-[14px] font-semibold tracking-tight text-text-1">
        <LuInfo size={14} className="mt-0.5 flex-none text-text-3" />
        {q}
      </h4>
      <p className="mt-1.5 pl-6 text-[13px] leading-relaxed text-text-2">
        {children}
      </p>
    </div>
  );
...
function Callout({
  ...
  const Icon =
    tone === "warn" ? LuTriangleAlert : tone === "accent" ? LuSparkles : LuInfo;
  const color =
    tone === "warn"
      ? "var(--warn)"
      : tone === "accent"
...
function Mono({ children }: { children: React.ReactNode }) {
  return (
    <span className="font-mono text-[0.92em] text-text-1">{children}</span>
  );
}
...
function SoonTag() {
  return (
    <span
      className="mr-1.5 inline-block rounded px-1.5 py-0.5 align-middle text-[10px] font-semibold uppercase tracking-widest"
